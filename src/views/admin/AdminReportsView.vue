...
const form = reactive({ status: 'RESOLVED', reviewNote: '' })
...
function handle(row: Report, value: string) {
  selected.value = row
  form.status = value
  form.reviewNote = ''
  open.value = true
...
async function submit() {
  ...
  try {
    await adminApi.handleReport(selected.value.id, form)
        ...
        <el-table-column label="操作" width="180" fixed="right" align="center">
          <template #default="{ row }">
            <template v-if="row.status === 'PENDING'">
              ...
              <el-button text @click="handle(row, 'DISMISSED')">
                忽略
              </el-button>
            </template>
          </template>
      ...
      <el-form label-position="top">
        <el-form-item label="处理结论">
          <el-segmented
            v-model="form.status"
            :options="[
              { label: '确认处理', value: 'RESOLVED' },
              { label: '驳回举报', value: 'DISMISSED' },
            ]"
          />
        </el-form-item>
