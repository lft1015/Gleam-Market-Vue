...
const form = reactive({ nickname: '', email: '', phone: '', avatar: '' })
...
async function load() {
  ...
  avatars.value = p.avatar ? [p.avatar] : []
}
    ...
    <div class="surface form-surface">
      <el-form label-position="top">
        <el-form-item label="头像">
          <ImageUploader v-model="avatars" :limit="1" />
        </el-form-item>
        ...
        <div
          v-if="auth.profile"
          style="display: flex; gap: 12px; align-items: center; margin-bottom: 18px; color: var(--ink-500)"
        >
          <StatusTag :status="auth.profile.status" />
          <span>注册于 {{ formatTime(auth.profile.createTime) }}</span>
        </div>
