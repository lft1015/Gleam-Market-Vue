...
async function choose(event: Event) {
  ...
  if (unsupported) {
    ElMessage.warning(
      `图片"${unsupported.name}"格式不受支持，请选择 JPEG、PNG、WebP 或 GIF 图片`,
    )
    input.value = ''
    return
  }
  ...
  try {
    const uploaded = await Promise.all(
      files.map((file) => commonApi.upload(file)),
    )
    emit(
      'update:modelValue',
      [...props.modelValue, ...uploaded.map((item) => item.url)],
    )
  } catch {
